"use client";

import { useState } from "react";
import { Cell, flexRender, getCoreRowModel, Row, useReactTable } from "@tanstack/react-table";
import { Table, TableHeader, TableBody, TableRow, TableHead, TableCell } from "@/components/ui/table";
import { ArrowRight, LoaderCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Pagination } from "@/components/layouts/Pagination";
import { BookingDialog } from "./BookingDialog";
import { useBookings, } from "../hooks/useBookings";
import { Booking } from "../types/booking.types";
import { Badge } from "@/components/ui/badge";
import { formatDate } from "@/helpers/date.helper";
import { formatCurrency } from "@/helpers/string.helper";

export const BookingTable = () => {
    const [page, setPage] = useState(1);
    const [limit, setLimit] = useState(10);
    const [selectedBooking, setSelectedBooking] = useState<Booking | null>(null);
    const [dialogOpen, setDialogOpen] = useState(false);

    const { data, isLoading } = useBookings(page, limit);
    // const deleteMutation = useDeleteBooking();

    const table = useReactTable({
        data: data?.data ?? [],
        columns: [
            { accessorKey: "room.name", header: "Room" },
            { accessorKey: "room.room_type.name", header: "Room Type" },
            {
                accessorKey: "total_price",
                header: () => <div className="text-right">Total Amount</div>,
            },
            { accessorKey: "status", header: "Status" },
            { accessorKey: "start_datetime", header: "Check In" },
            { accessorKey: "end_datetime", header: "Check Out" },
            { accessorKey: "action", header: "" },
        ],
        pageCount: Math.ceil((data?.meta.total ?? 0) / limit),
        manualPagination: true,
        state: {
            pagination: { pageIndex: page - 1, pageSize: limit },
        },
        getCoreRowModel: getCoreRowModel(),
    });

    const handleBookingClick = (booking: Booking) => {
        console.log('booking', booking)
        if (booking.id) {
            setSelectedBooking(booking);
            setDialogOpen(true);
        }
    }

    const renderCellContent = (index: number, row: Row<Booking>, cell: Cell<Booking, unknown>) => {
        switch (true) {
            case cell.column.id === "start_datetime":
            case cell.column.id === "end_datetime":
                return (
                    <TableCell key={cell.id}>
                        {formatDate(cell.getValue() as string, "MM/DD/YYYY hh:mm A")}
                    </TableCell>
                )
            case cell.column.id === 'total_price':
                return (
                    <TableCell key={cell.id} className="text-right">
                        {formatCurrency(cell.getValue() as number, { currencySymbol: "" })}
                    </TableCell>
                )
            case index === 6:
                return (
                    <TableCell key={cell.id}>
                        <Button
                            variant="ghost"
                            className="text-gray-400 hover:text-gray-400 cursor-pointer float-right size-7"
                            onClick={() => {
                                handleBookingClick(row.original);
                            }}
                        >
                            <ArrowRight className="w-4 h-4" />
                        </Button>
                    </TableCell>

                )

            case cell.column.id === "status":
                const cellValue = cell.getValue() as string;
                return (
                    <TableCell key={cell.id}>
                        <Badge
                            variant={cellValue === "check_in" ? "default" : "destructive"}
                            className={cellValue === "check_in" ?
                                "bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300" :
                                "bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300"
                            }
                        >
                            {cellValue === 'check_in' ? 'Check In' : 'Checked Out'}
                        </Badge>
                    </TableCell>
                );


            default:
                return (
                    <TableCell key={cell.id}>
                        {flexRender(cell.column.columnDef.cell, cell.getContext())}
                    </TableCell>
                )
        }
    };

    return (
        <div className="md:max-w-7xl mx-auto">
            <BookingDialog
                mode="edit"
                open={dialogOpen}
                onOpenChange={setDialogOpen}
                bookingId={selectedBooking?.id ?? 0}
            />

            <Table className="bg-white">
                <TableHeader>
                    {table.getHeaderGroups().map(hg => (
                        <TableRow key={hg.id}>
                            {hg.headers.map((header) =>
                                <TableHead key={header.id} className="text-xs text-gray-600">
                                    {flexRender(header.column.columnDef.header, header.getContext())}
                                </TableHead>
                            )}
                        </TableRow>
                    ))}
                </TableHeader>

                <TableBody>
                    {isLoading ? (
                        <TableRow>
                            <TableCell colSpan={7} className="text-center">
                                <LoaderCircle className="w-6 h-6 animate-spin mx-auto" />
                            </TableCell>
                        </TableRow>
                    ) : table.getRowModel().rows.length ? (
                        table.getRowModel().rows.map(row => (
                            <TableRow
                                key={row.id}
                                className="cursor-pointer"
                                onClick={() => handleBookingClick(row.original)}
                            >
                                {row.getVisibleCells().map((cell, index) => {
                                    return renderCellContent(index, row, cell)
                                })}
                            </TableRow>
                        ))
                    ) : (
                        <TableRow>
                            <TableCell colSpan={7} className="text-center text-xs italic text-gray-400">
                                No record
                            </TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>

            {!isLoading && data && (
                <Pagination
                    table={table}
                    onChangePage={setPage}
                    onChangeLimit={(newLimit) => {
                        setLimit(newLimit);
                        setPage(1);
                    }}
                />
            )}
        </div>
    );
};
