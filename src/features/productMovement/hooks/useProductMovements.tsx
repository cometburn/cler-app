...
const PRODUCTS_MOVEMENT_QUERY_KEY = ["product_movements"];
...
export function useProductMovements(page: number, limit: number, category?: string, search?: string) {
    return useQuery<ProductMovementResponse, Error>({
        queryKey: [...PRODUCTS_MOVEMENT_QUERY_KEY, page, limit, category, search],
        queryFn: () => {
    ...
    return useMutation<ProductMovementResponse, ApiError, ProductMovement>({
        ...
        onSettled: () => {
            queryClient.invalidateQueries({
                queryKey: PRODUCTS_MOVEMENT_QUERY_KEY,
            });
        },
    });
}
...
/* ================================
   Update Product 
================================ */
export function useUpdateProductMovement() {
    const queryClient = useQueryClient();

    return useMutation<ProductMovementResponse, ApiError, ProductMovement>({
        mutationFn: updateProductMovement,
